...
@testset "Testing `complex.jl`" begin
    ...
    @testset "Test integration of ODE with complex dependent variables (1)" begin
        ...
        zsol1 = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs1, z0, ts, _order, _abstol, maxsteps=1))
        @test size(zsol1) == (length(ts),)
        zsol1 = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs1, z0, tr, _order, _abstol, maxsteps=1, nothing))
        @test length(zsol1) == length(tr)
        ta = vec(tr)
        zsol1 = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs1, z0, ta, _order, _abstol, maxsteps=1))
        @test length(zsol1) == length(ta)
        zsol1 = (@test_logs min_level=Logging.Warn taylorinteg(
            eqs1, z0, tr, _order, _abstol))
        @test zsol1[1] == z0
        @test isapprox( zsol1[2]  , z0*exp(-tr[2]) )
        @test isapprox( zsol1[6], z0*exp(-tr[6]) )
        @test isapprox( zsol1[end], z0*exp(-tr[end]) )
        zsol11 = (@test_logs min_level=Logging.Warn taylorinteg(
            eqs1, z0, ta, _order, _abstol, nothing))
        @test zsol11 == zsol1
        @test zsol11[1] == z0
        @test isapprox( zsol11[2]  , z0*exp(-tr[2]) )
        @test isapprox( zsol11[6], z0*exp(-tr[6]) )
        @test isapprox( zsol11[end], z0*exp(-tr[end]) )
        tt, zsol1t = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs1, z0, 0.0, 2pi, _order, _abstol, maxsteps=1))
        @test length(tt) == 2
        @test length(zsol1t) == 2
        tt, zsol1t = (@test_logs min_level=Logging.Warn taylorinteg(
            eqs1, z0, 0.0, 2pi, _order, _abstol))
        @test zsol1t[1] == z0
    ...
    @testset "Test integration of ODE with complex dependent variables (2)" begin
        ...
        zsol2 = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs2, z0, ts, _order, _abstol, maxsteps=1, nothing))
        @test size(zsol2) == (length(ts),)
        zsol2 = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs2, z0, tr, _order, _abstol, maxsteps=1))
        @test length(zsol2) == length(tr)
        ta = vec(tr)
        zsol2 = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs2, z0, ta, _order, _abstol, maxsteps=1))
        @test length(zsol2) == length(ta)
        zsol2 = (@test_logs min_level=Logging.Warn taylorinteg(
            eqs2, z0, tr, _order, _abstol))
        @test zsol2[1] == z0
        @test isapprox( zsol2[3], z0*exp( complex(0.0, tr[3])) )
        @test isapprox( zsol2[5], z0*exp( complex(0.0, tr[5])) )
        zsol22 = (@test_logs min_level=Logging.Warn taylorinteg(
            eqs2, z0, ta, _order, _abstol, nothing))
        @test zsol22 == zsol2
        @test zsol22[1] == z0
        @test isapprox( zsol22[3], z0*exp( complex(0.0, tr[3])) )
        @test isapprox( zsol22[5], z0*exp( complex(0.0, tr[5])) )
        tt, zsol2 = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs2, z0, 0.0, 2pi, _order, _abstol, maxsteps=1))
        @test length(tt) == 2
        @test length(zsol2) == 2
        tt, zsol2 = (@test_logs min_level=Logging.Warn taylorinteg(
            eqs2, z0, 0.0, 2pi, _order, _abstol))
        @test zsol2[1] == z0
    ...
    @testset "Test integration of ODE with complex dependent variables (3)" begin
        ...
        zsol33 = (@test_logs min_level=Logging.Warn taylorinteg(
            eqs3!, [z0, z0], ta, _order, _abstol, nothing))
        @test zsol33 == zsol3
        ...
        tt, zsol3 = (@test_logs (Warn, max_iters_reached()) taylorinteg(
            eqs3!, zz0, 0.0, 2pi, _order, _abstol, maxsteps=1))
        @test length(tt) == 2
        @test size(zsol3) == (length(tt), length(zz0))
        tt, zsol3 = (@test_logs min_level=Logging.Warn taylorinteg(
            eqs3!, zz0, 0.0, 2pi, _order, _abstol))
        @test zsol3[1,1] == z0
        @test zsol3[1,2] == z0
