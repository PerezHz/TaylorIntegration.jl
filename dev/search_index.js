...

{
    ...
    "title": "Poincaré maps",
    "category": "section",
    "text": "In this example, we shall illustrate how to construct a Poincaré map associated with the surface of section x=0, dot x0, for E=01025 for the Hénon-Heiles system. This is equivalent to find the roots of an appropriate function g(t, x, dx). We illustrate the implementation using many initial conditions (Monte Carlo like implementation), and then compare the results with the use of jet transport techniques."
},
...

{
    ...
    "title": "An example",
    "category": "section",
    "text": "In order to explain how the macro works, we shall use as an example the mathematical pendulum. First, we carry out the integration using the default method, as described before.using TaylorIntegration\n\nfunction pendulum!(t, x, dx)\n    dx[1] = x[2]\n    dx[2] = -sin(x[1])\n    return dx\nend\n\n# Initial time (t0), final time (tf) and initial condition (q0)\nt0 = 0.0\ntf = 100.0\nq0 = [1.3, 0.0]\n\n# The actual integration\nt1, x1 = taylorinteg(pendulum!, q0, t0, tf, 28, 1e-20, maxsteps=1500); # warm-up run\ne1 = @elapsed taylorinteg(pendulum!, q0, t0, tf, 28, 1e-20, maxsteps=1500);\ne1We note that the initial number of methods defined for TaylorIntegration.jetcoeffs! is 2.length(methods(TaylorIntegration.jetcoeffs!)) == 2 # initial valueUsing @taylorize will increase this number by creating a new method.The macro @taylorize is intended to be used in front of the function that implements the equations of motion. The macro does the following: it first evaluates the actual function as it is, so the integration can be computed using taylorinteg as above, or by explicitly using the keyword argument parse_eqs=false. It then creates and evaluates a new method of TaylorIntegration.jetcoeffs!, which is the specialized method (through Val) on the specific function passed to the macro.@taylorize function pendulum!(t, x, dx)\n    dx[1] = x[2]\n    dx[2] = -sin(x[1])\n    return dx\nend\n\nprintln(methods(pendulum!))println(methods(TaylorIntegration.jetcoeffs!))We see that there is only one method of pendulum!, and there is a new method of TaylorIntegration.jetcoeffs!, whose signature appears in this documentation as Val{Main.ex-taylorize.pendulum!}; it is an specialized version for the function pendulum! (with some extra information about the module where the function was created). This method is selected internally if it exists by default, exploiting dispatch, when calling taylorinteg or lyap_taylorinteg; to integrate using the hard-coded method of TaylorIntegration.jetcoeffs! of the integration above, the keyword argument parse_eqs has to be set to false.Now we carry out the integration using the specialized method; note that we use the same instruction as above.t2, x2 = taylorinteg(pendulum!, q0, t0, tf, 28, 1e-20, maxsteps=1500); # warm-up run\ne2 = @elapsed taylorinteg(pendulum!, q0, t0, tf, 28, 1e-20, maxsteps=1500);\ne2We note the striking difference in the performance:e1/e2We can check that both integrations yield the same results.t1 == t2 && x1 == x2The speed-up obtained comes from the design of the new (specialized) method of TaylorIntegration.jetcoeffs! as described above: it avoids some allocations and some repeated computations. This is achieved by knowing the specific AST of the function of the ODEs integrated, which is walked through and translated into the actual implementation, where some required auxiliary arrays are created and the low-level functions defined in TaylorSeries.jl are used. For this, we heavily rely on Espresso.jl and some metaprogramming; we thank Andrei Zhabinski for his help and comments.The new jetcoeffs! method can be obtained as follows:ex = :(function pendulum!(t, x, dx)\n    dx[1] = x[2]\n    dx[2] = -sin(x[1])\n    return dx\nend)\n\nnew_ex = TaylorIntegration._make_parsed_jetcoeffs(ex)This function has a similar structure as the hard-coded method of TaylorIntegration.jetcoeffs!, but uses low-level functions in TaylorSeries (e.g., sincos! above) and explicitly allocates the needed temporary arrays. More complex functions become easily very difficult to read. Note that, if necessary, one can further optimize new_ex manually.As stated above, in order to allow to opt-out from using the specialized method created by @taylorize, taylorinteg and lyap_taylorinteg recognize the keyword argument parse_eqs; setting it to false imposes using the standard method.taylorinteg(pendulum!, q0, t0, tf, 28, 1e-20, maxsteps=1500, parse_eqs=false); # warm-up run\n\ne3 = @elapsed taylorinteg(pendulum!, q0, t0, tf, 28, 1e-20, maxsteps=1500, parse_eqs=false);e1/e3"
},

{
    "location": "taylorize/#Limitations-1",
    "page": "Optimizing: @taylorize",
    "title": "Limitations",
    "category": "section",
    "text": "The construction of the function by using @taylorize is somewhat complicated and limited. It is useful to have expressions which involve two arguments at most, which imposes the proper use of parenthesis: For example, a+b+c should be written as (a+b)+c. The macro allows to use array declarations through Array, but other ways (e.g. similar) are not yet implemented. It is recommended to not use variables prefixed by an underscore, in particular _T and _S, to avoid name collisions. Broadcasting is not recognized by @taylorize, and the macro can\'t be used when the integration is performed through the interoperability with DifferentialEquations.jl. if-else blocks are recognized in its long form, but short-circuit conditional operators (&& and ||) are not. Expressions which correspond to function calls (so the head field is :call) which are not recognized by the parser are simply copied. The heuristics used, specially for vectors, may not work for all cases.It is recommended to skim test/taylorize.jl, which implements different cases, including uses of local for internal parameters, which in some cases adds some performance.Please report any problems you may encounter."
},
...
{
    "location": "api/#TaylorIntegration.__jetcoeffs!-Tuple{Val{false},Any,Any,Any}",
    "page": "API",
    "title": "TaylorIntegration.__jetcoeffs!",
    "category": "method",
    "text": "__jetcoeffs!(::Val{bool}, f, t, x)\n__jetcoeffs!(::Val{bool}, f, t, x, dx, xaux)\n\nChooses a method of jetcoeffs! (hard-coded or generated by @taylorize) depending on bool::Bool.\n\n\n\n\n\n"
},
...
{
    "location": "api/#TaylorIntegration.findroot!-NTuple{19,Any}",
    "page": "API",
...
{
    "location": "api/#TaylorIntegration.lyap_taylorstep!-Union{Tuple{U}, Tuple{T}, Tuple{Any,Taylor1{T},Array{Taylor1{U},1},Array{Taylor1{U},1},Array{Taylor1{U},1},Array{TaylorN{Taylor1{U}},1},Array{TaylorN{Taylor1{U}},1},Array{Taylor1{U},2},T,T,Array{U,1},Int64,T,Array{TaylorN{Taylor1{U}},1},Array{Taylor1{U},3}}, Tuple{Any,Taylor1{T},Array{Taylor1{U},1},Array{Taylor1{U},1},Array{Taylor1{U},1},Array{TaylorN{Taylor1{U}},1},Array{TaylorN{Taylor1{U}},1},Array{Taylor1{U},2},T,T,Array{U,1},Int64,T,Array{TaylorN{Taylor1{U}},1},Array{Taylor1{U},3},Bool}, Tuple{Any,Taylor1{T},Array{Taylor1{U},1},Array{Taylor1{U},1},Array{Taylor1{U},1},Array{TaylorN{Taylor1{U}},1},Array{TaylorN{Taylor1{U}},1},Array{Taylor1{U},2},T,T,Array{U,1},Int64,T,Array{TaylorN{Taylor1{U}},1},Array{Taylor1{U},3},Bool,Any}} where U<:Number where T<:Real",
    "page": "API",
...
{
    "location": "api/#TaylorIntegration.stabilitymatrix!-Union{Tuple{U}, Tuple{T}, Tuple{Any,Taylor1{T},Array{Taylor1{U},1},Array{TaylorN{Taylor1{U}},1},Array{TaylorN{Taylor1{U}},1},Array{Taylor1{U},2},Array{TaylorN{Taylor1{U}},1}}, Tuple{Any,Taylor1{T},Array{Taylor1{U},1},Array{TaylorN{Taylor1{U}},1},Array{TaylorN{Taylor1{U}},1},Array{Taylor1{U},2},Array{TaylorN{Taylor1{U}},1},Any}} where U<:Number where T<:Real",
    "page": "API",
...
{
    "location": "api/#TaylorIntegration.surfacecrossing-Union{Tuple{T}, Tuple{Taylor1{T},Taylor1{T},Int64}} where T<:Number",
    "page": "API",
