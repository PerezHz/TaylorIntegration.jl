doc"""`ad!{T<:Number}(f, x::Array{Taylor1{T},1}, order::Int, params)`

`ad!` calculates the recursion relation of derivatives for the ODE `ẋ=f(x)`, with
...
doc"""`taylor_one_step!{T<:Number}(f, timestep_method, elaptime::Ref{T}, state::Array{T,1}, abs_tol::T, order::Int, params)`
...
"""
function taylor_one_step!{T<:Number}(f, timestep_method, elaptime::Ref{T}, state::Array{T,1}, abs_tol::T, order::Int, params)
...
doc"""`taylor_integrator!{T<:Number}(f, timestep_method, initial_state::Array{T,1}, abs_tol::T, order::Int, t_max::T, datalog::Array{Array{T,1},1}, params...)`
...
in the equations of motion as well.
"""
function taylor_integrator!{T<:Number}(f, timestep_method, initial_state::Array{T,1},
    ...
    elapsed_time = Ref(zero(T)) #this `Base.RefValue{T}` variable stores elapsed time, so that we can change its .x field inside `taylor_one_step!`
    ...
    while datalog[1][end]<t_max
        ...
        state = taylor_one_step!(f, timestep_method, elapsed_time, state, abs_tol, order, params)#::Array{T,1}
...
doc"""`taylor_integrator_k!{T<:Number}(f, timestep_method, initial_state::Array{T,1}, abs_tol::T, order::Int, t_max::T, datalog::Array{Array{T,1},1}, k_max::Int, params...)`
...
first component of the `initial_state` array, and its evolution ṫ=1 must be included
in the equations of motion as well."""
function taylor_integrator_k!{T<:Number}(f, timestep_method, initial_state::Array{T,1},
    abs_tol::T, order::Int, t0::T, t_max::T, datalog::Array{Array{T,1},1},
    k_max::Int, params...)
...
function taylor_one_step_v2!{T<:Number}(f, timestep_method,
    ...
    step = timestep_method(stateT, abs_tol)::T
    new_state = taylor_propagator(order, step, stateT, params)::Array{T,1}
