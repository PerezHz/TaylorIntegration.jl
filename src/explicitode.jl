...
# taylorinteg
@doc doc"""
    taylorinteg(f, x0, t0, tmax, order, abstol, params; kwargs... )

General-purpose Taylor integrator for the explicit ODE
``\dot{x}=f(t, x, p)``, with initial condition specified by `x0` at time `t0`,
and any parameters encoded in `params` whose default is `nothing`.
The initial condition `x0` may be of type `T<:Number`
