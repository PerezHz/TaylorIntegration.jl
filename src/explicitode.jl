...
# __jetcoeffs
"""
    __jetcoeffs!(::Val{bool}, f, t, x, params, tmpTaylor, arrTaylor)
    __jetcoeffs!(::Val{bool}, f, t, x, dx, xaux, params, tmpTaylor, arrTaylor)
...
@inline __jetcoeffs!(::Val{true},  f, t, x::Taylor1{U}, params, tmpTaylor, arrTaylor) where {U} =
    jetcoeffs!(Val(f), t, x, params, tmpTaylor, arrTaylor)

@inline __jetcoeffs!(::Val{false}, f, t, x::Array{Taylor1{U},1}, dx, xaux, params) where {U} =
    jetcoeffs!(f, t, x, dx, xaux, params)
@inline __jetcoeffs!(::Val{true},  f, t, x::Array{Taylor1{U},1}, dx, params, tmpTaylor, arrTaylor) where {U} =
    jetcoeffs!(Val(f), t, x, dx, params, tmpTaylor, arrTaylor)


# _allocate_jetcoeffs! fallbacks; needed to define the corresponding parsed method
_allocate_jetcoeffs!(::Taylor1{T}, x::Taylor1{S}, params) where {T,S} =
    (typeof(x)[], Vector{typeof(x)}[])
_allocate_jetcoeffs!(::Taylor1{T}, x::AbstractArray{Taylor1{S}, N},
        ::AbstractArray{Taylor1{S}, N}, params) where {T,S,N} =
    (eltype(x)[], Vector{eltype(x)}[])
...
function taylorstep!(f, t::Taylor1{T}, x::Taylor1{U}, abstol::T, params,
        tmpTaylor::Array{Taylor1{U},1}, arrTaylor) where {T<:Real, U<:Number}

    # Compute the Taylor coefficients
    __jetcoeffs!(Val(true), f, t, x, params, tmpTaylor, arrTaylor)
...
function taylorstep!(f!, t::Taylor1{T}, x::Vector{Taylor1{U}},
    ...
    # Compute the Taylor coefficients
    __jetcoeffs!(Val(true), f!, t, x, dx, params, tmpTaylor, arrTaylor)
...
function _determine_parsing!(parse_eqs::Bool, f, t, x, params)
    ...
    if parse_eqs
        try
            tmpTaylor, arrTaylor = _allocate_jetcoeffs!(Val(f), t, x, params)
            jetcoeffs!(Val(f), t, x, params, tmpTaylor, arrTaylor)
        catch
...
function _determine_parsing!(parse_eqs::Bool, f, t, x, dx, params)
    ...
    tmpTaylor, arrTaylor = _allocate_jetcoeffs!(t, x, dx, params)

    if parse_eqs
        try
            tmpTaylor, arrTaylor = _allocate_jetcoeffs!(Val(f), t, x, dx, params)
            jetcoeffs!(Val(f), t, x, dx, params, tmpTaylor, arrTaylor)
        catch
            @warn("""Unable to use the parsed method of `jetcoeffs!` for `$f`,
            despite of having `parse_eqs=true`, due to some internal error.
    ...
    @eval begin
        function taylorinteg(f, x0::U, t0::T, tmax::T, order::Int, abstol::T, ::$V, params = nothing;
            ...
            if parse_eqs
                ...
                t = t0 + Taylor1( T, order )
                x = Taylor1( x0, order )
                return _taylorinteg!(f, t, x, x0, t0, tmax, abstol, tmpTaylor, arrTaylor,
        ...
        function _taylorinteg!(f, t::Taylor1{T}, x::Taylor1{U},
            ...
            # Integration
            while sign_tstep*t0 < sign_tstep*tmax
                δt = taylorstep!(f, t, x, abstol, params, tmpTaylor, arrTaylor) # δt is positive!
                # Below, δt has the proper sign according to the direction of the integration
                δt = sign_tstep * min(δt, sign_tstep*(tmax-t0))
                x0 = evaluate(x, δt) # new initial condition
                if $V == Val{true}
        ...
        function taylorinteg(f!, q0::Array{U,1}, t0::T, tmax::T, order::Int, abstol::T, ::$V, params = nothing;
            ...
            if parse_eqs
                ...
                t = t0 + Taylor1( T, order )
                x .= Taylor1.( q0, order )
                return _taylorinteg!(f!, t, x, dx, q0, t0, tmax, abstol, tmpTaylor, arrTaylor,
                    $V(), params, maxsteps=maxsteps)
            else
...
function taylorinteg(f, x0::U, trange::AbstractVector{T},
    ...
    if parse_eqs
        return _taylorinteg!(f, t, x, x0, trange, abstol, tmpTaylor, arrTaylor,
...
function taylorinteg(f!, q0::Array{U,1}, trange::AbstractVector{T},
    ...
    if parse_eqs
        return _taylorinteg!(f!, t, x, dx, q0, trange, abstol, tmpTaylor, arrTaylor,
            params, maxsteps=maxsteps)
    else
