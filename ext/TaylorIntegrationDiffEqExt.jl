...
ODEqCore.alg_order(alg::TaylorMethodParams) = alg.order
...
### cache stuff
struct TaylorMethodCache{uType,rateType,tTType,uTType} <: ODEqCore.OrdinaryDiffEqMutableCache
    u::uType
    uprev::uType
    tmp::uType
    k::rateType
...
struct TaylorMethodConstantCache{uTType} <: ODEqCore.OrdinaryDiffEqConstantCache
    uT::uTType
    parse_eqs::Ref{Bool}
    rv::RetAlloc{uTType}
end

function ODEqCore.alg_cache(
    alg::TaylorMethodParams,
    u,
    rate_prototype,
    uEltypeNoUnits,
...
function ODEqCore._ode_addsteps!(
    k,
    t,
    uprev,
    u,
    ...
    if length(k) < 2 || always_calc_begin
        if typeof(cache) <: ODEqCore.OrdinaryDiffEqMutableCache
...
function DiffEqBase.interp_summary(
    ::Type{cacheType},
    dense::Bool,
) where {cacheType<:TaylorMethodCache}
...
) where {U} = TaylorIntegration.__jetcoeffs!(Val(false), f.f, t, x, params, rv)
@inline TaylorIntegration.__jetcoeffs!(
    ::Val{true},
    f::ODEFunction,
    t,
    ...
    params,
    rv::RetAlloc{Taylor1{U}},
) where {U} = TaylorIntegration.__jetcoeffs!(Val(true), f.f, t, x, params, rv)
@inline TaylorIntegration.__jetcoeffs!(
    ::Val{false},
    f::ODEFunction,
    t,
    ...
    params,
    rv::RetAlloc{Taylor1{U}},
) where {U,N} = TaylorIntegration.__jetcoeffs!(Val(false), f.f, t, x, dx, xaux, params, rv)
@inline TaylorIntegration.__jetcoeffs!(
    ::Val{true},
    f::ODEFunction,
    t,
    ...
    params,
    rv::RetAlloc{Taylor1{U}},
) where {U,N} = TaylorIntegration.__jetcoeffs!(Val(true), f.f, t, x, dx, xaux, params, rv)
...
@inline TaylorIntegration.__jetcoeffs!(
    ...
    params,
    rv::RetAlloc,
) = TaylorIntegration.jetcoeffs!(f, t, vec(x), vec(dx), xaux, params)
